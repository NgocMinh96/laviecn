...
const useFluidCursor = () => {
  ...
  const pointers: Pointer[] = [new PointerPrototype()]
  ...
  function getWebGLContext(canvas: HTMLCanvasElement): WebGLContext {
    ...
    let gl: WebGLRenderingContext | WebGL2RenderingContext = canvas.getContext(
      "webgl2",
      params
    ...
    let halfFloat: WebGLExtension | null = null
    ...
    if (!halfFloat && !isWebGL2) {
      throw new Error("Half float texture not supported")
    }
    ...
    if (isWebGL2) {
      const gl2 = gl as WebGL2RenderingContext
      formatRGBA =
        getSupportedFormat(gl2, gl2.RGBA16F, gl2.RGBA, halfFloatTexType) ||
        formatRGBA
      formatRG =
        getSupportedFormat(gl2, gl2.RG16F, gl2.RG, halfFloatTexType) || formatRG
      formatR =
        getSupportedFormat(gl2, gl2.R16F, gl2.RED, halfFloatTexType) || formatR
    }
...
`
  ...
  return {
    // Export any necessary functions or values
  }
}
